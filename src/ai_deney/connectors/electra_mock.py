"""Deterministic mock connector for Electra exports."""
...
class ElectraMockConnector(ElectraConnectorBase):
    """
    Resolve Electra reports from local fixtures and copy them to deterministic
    raw-data locations.
    """
    ...
    def __init__(
        ...
        self.fixture_root = (fixture_root or (self.repo_root / "fixtures" / "electra")).resolve()
        self.raw_root = (raw_root or (self.repo_root / "data" / "raw" / "electra_mock")).resolve()
    ...
    def fetch_report(self, report_type: str, params: dict) -> list[Path]:
        ...
        - ``include_pdf_sample``: bool (only for ``sales_summary``)
        """
        if report_type not in _SUPPORTED_REPORT_TYPES:
            raise ValueError(f"unsupported report_type: {report_type}")

        years = self._coerce_years(params)
        include_pdf_sample = bool(params.get("include_pdf_sample", False))
        out_paths: list[Path] = []

        for year in years:
            fixture_name = f"{report_type}_{year}.csv"
        ...
        if report_type == "sales_summary" and include_pdf_sample:
            pdf_src = self.fixture_root / "sales_summary_sample.pdf"
            if not pdf_src.exists():
                raise FileNotFoundError(f"fixture not found: {pdf_src}")
            pdf_target_dir = self.raw_root / report_type / "pdf_samples"
            _ensure_within_root(pdf_target_dir, self.repo_root)
            pdf_target_dir.mkdir(parents=True, exist_ok=True)
            pdf_dst = pdf_target_dir / pdf_src.name
            shutil.copyfile(pdf_src, pdf_dst)
            out_paths.append(pdf_dst)
